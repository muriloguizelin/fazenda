...
export async function registerLoteRoutes(app: FastifyInstance) {
  ...
  app.get('/lotes/:id', { preHandler: [authGuard] }, async (req: any) => {
    ...
    if (!lote) {
      throw { statusCode: 404, response: { error: { code: 'NOT_FOUND', message: 'Lote não encontrado' } } };
    }
    return lote;
  });

  app.post('/lotes', { preHandler: [authGuard] }, async (req: any, reply) => {
    const schema = z.object({ fazendaId: z.string(), nome: z.string().min(1), prefixo: z.string().optional(), capacidade: z.number().int().optional() });
    const data = schema.parse(req.body);
    const created = await app.prisma.lote.create({ data });
    return reply.code(201).send(created);
  });

  app.put('/lotes/:id', { preHandler: [authGuard] }, async (req: any) => {
    const { id } = req.params as any;
    const schema = z.object({ nome: z.string().min(1).optional(), prefixo: z.string().optional(), capacidade: z.number().int().optional() });
    const data = schema.parse(req.body);
    const updated = await app.prisma.lote.update({ where: { id }, data });
    return updated;
  });

  app.delete('/lotes/:id', { preHandler: [authGuard] }, async (req: any, reply) => {
    const { id } = req.params as any;
